...
function abstract_call_method(interp::AbstractInterpreter, method::Method, @nospecialize(sig), sparams::SimpleVector, hardlimit::Bool, si::StmtInfo, sv::AbsIntState)
...
function edge_matches_sv(frame::AbsIntState, method::Method, @nospecialize(sig),
                         sparams::SimpleVector, hardlimit::Bool, sv::AbsIntState,
    ...
    if !hardlimit || ignore_recursion_hardlimit
        ...
        if code !== nothing
            ...
            if irsv !== nothing
                ...
                if !(isa(rt, Type) && hasintersect(rt, Bool))
                    ir = irsv.ir
                    # TODO double inlining pass when there are any newly resolved calls
